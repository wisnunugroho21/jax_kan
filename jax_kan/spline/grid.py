...
class SplineGrid(Grid):
    """
    SplineGrid class, corresponding to the grid of the BaseLayer class. It comprises an initialization as well as an update procedure.
    ...
    def __init__(
        ...
        Example:
            >>> grid_type = SplineGrid(n_in = 2, n_out = 5, k = 3, G = 3, grid_range = (-1,1), grid_e = 0.05)
            >>> grid = grid_type.item
        """
    ...
    def _initialize(self):
        ...
        grid = (
            jnp.arange(-self.k, self.G + self.k + 1, dtype=jnp.float32) * h
            + self.grid_range[0]
        )
    ...
    def update(self, x: jax.Array, G_new: int) -> None:
        ...
        grid_uniform = (
            jnp.arange(G_new + 1, dtype=jnp.float32) * uniform_step[:, None]
            + x_sorted[:, 0][:, None]
            - margin
        )
